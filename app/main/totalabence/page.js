...
export default function DelayEarlyLeaveReport() {
    ...
    let show = searched.map((el) => {
      return (
        <tr className=" grid-cols-11 border">
          ...
          <td className=" col-span-2  p-2">{el.day.substr(0, 10)}</td>
        </tr>
      );
    });
        ...
        >
          <thead>
            ...
            <tr className=" grid-cols-11  w-full bg-[#8c929450] m-1 text-black/70 border">
              <th colspan={6} className=" col-span-11">
                {isArabicprop
  ...
  const printDocument = () => {
    ...
    doc.save("تقرير اجمالي الغياب.pdf");
  };
    ...
    <div className=" font-sans">
      ...
      <div>
        <Label
          headers={headers}
          data={cvsData}
          fileName="تقارير اجمالي الغياب "
          pdf={printDocument}
          setsearch={showSearchHandeller}
          label={isArabicprop ? " اجمالي الغياب" : "Total Absence"}
        />
      </div>
      {showSearch && (
        <div className=" mb-12">
