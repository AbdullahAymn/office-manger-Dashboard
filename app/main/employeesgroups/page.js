...
import { tempGroupData } from "./tempdata";
...
export default function page() {
  ...
  const [groupsData, setGroupsData] = useState(tempGroupData);
  ...
  const tabelData = slice.map((e) => (
    <tr key={e.id} className="grid grid-cols-9 p-2">
      <td className=" col-span-4 md:col-span-6 text-start">{ isArabicprop ? e.nameAr : e.nameEn}</td>
          ...
          <table className=" min-w-full">
            <thead>
              <tr className=" grid grid-cols-9 bg-white p-2 border text-black/70">
                <th className=" col-span-4 md:col-span-6 text-start">
