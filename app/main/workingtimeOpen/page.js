...
export default function WorkingTime() {
  ...
  useEffect(() => {
    setLoader(true);
    fetch(`https://backend2.dasta.store/api/auth/basicInfoFetchchangeshift`, {
      method: "GET",
      headers: myHeaders,
    }).then((res) => {
      if (res.status === 200) {
  ...
  const tabelData = slice.map((e) => (
    <tr key={e.id} className=" grid-cols-7 p-2">
      <td className=" col-span-3 text-start text-sky-800">
        <Link href={`/main/workingtimeOpen/${e.id}`}>
          {isArabicprop ? e.name : e.name_en}
        </Link>
      </td>
      <td className=" p-2 col-span-1 text-end text-black/70">
        <i
    ...
    <div className="overflow-x-hidden">
      ...
      <div>
        ...
        <NameAndSearch
          ...
          name={isArabicprop ? " أوقات العمل (مفتوح)" : "Shifts (open)"}
      ...
      <div>
        ...
        <Popup open={openDelet}>
          <Delete
            ...
            link={"basicInfoDeletechangeshift"}
            element={deleteditem}
          />
        </Popup>
        ...
        <div className=" w-full font-sans my-4 overflow-x-scroll md:overflow-x-hidden">
          <table className=" text-sm md:text-base min-w-full md:w-full">
            <thead>
              <tr className="  grid-cols-7 bg-white p-2 border text-black/70">
                <th className=" col-span-3 p-2 text-start">
                  {isArabicprop ? "الإسم" : " Name"}
                </th>
                <th className=" col-span-1 px-6 text-end">
